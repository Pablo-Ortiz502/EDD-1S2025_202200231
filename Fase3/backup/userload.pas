...
type
  UserLoader = class
  public
    class procedure readFile(const fileName: string; List: LinkedList);
  end;
...
class procedure UserLoader.readFile(const fileName: string; List: LinkedList);
var
  JSONData: TJSONData;
    ...
    try
      ...
      try
        ...
        begin
          ...
          NewUser.edad:=StrToInt(UserObject.Strings['edad']); // aqui se cambio lo de la edad, recuerda quitar lo de graphviz en unit 3 la lista
          List.add(NewUser);
        end;
      finally
        JSONData.Free;
