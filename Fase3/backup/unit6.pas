...
begin
  bLogUser := u;
  messList := bLogUser.messTree.InOrderList;
...
procedure TForm4.downButtonClick(Sender: TObject);
var
  j: Message;
begin
  if (MessageDlg('Quieres hacer este mensaje favorito?',mtWarning,[mbOk,mbCancel],0) = mrOk) then
  begin
   j := bLogUser.messTree.FindById(i);
     if j <> nil then
     begin
      j.DownloadCompressed;
      ShowMessage('Mensaje descargado');
     end;
  end;
end;

procedure TForm4.favButtonClick(Sender: TObject);
var
  m: Message;
begin
  if (MessageDlg('Quieres hacer este mensaje favorito?',mtWarning,[mbOk,mbCancel],0) = mrOk) then
...
begin
  messList := bLogUser.messTree.PostOrderList();
  Form4.refreshList(listView1);
end;

procedure TForm4.preButtonClick(Sender: TObject);
begin
  messList := bLogUser.messTree.PreOrderList();
  Form4.refreshList(listView1);
end;
