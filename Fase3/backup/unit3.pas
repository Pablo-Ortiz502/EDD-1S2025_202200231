...
uses
  Classes, SysUtils, Unit5,stackL,circularL,queuL,Process,avlTree,BTSTreeC, BTreeMessages,merckeTree;
...
type
    ...
    LinkedList = class
      ...
      public
        ...
        function findUser(auser: string): User;
      end;
...
{LinkedList}
     ...
     function LinkedList.findUser(auser: string): User;
       var
         current: Node;
       begin
         current := head;
         while current <> nil do
           begin
             if current^.data.user = auser then
               begin
                 Result := current^.data;
                 Exit;
               end;
