...
uses
  Classes, SysUtils, Process, MessageClasss, Unit5; // Unit5 = tu DoubleList

type
  // Nodo del árbol de Merkle
  TMerkleNode = class
  public
    HashValue: String;
    Left, Right: TMerkleNode;
    MsgData: Message; // solo en las hojas
...
begin
  ...
  begin
    Writeln('⚠️ No se puede construir árbol: la lista está vacía.');
    Exit;
  end;
  BuildTree(msgList);
end;
...
begin
  ...
  try
    ...
    begin
      ...
      Writeln('✅ Árbol de Merkle con un solo nodo. Hash raíz: ', Copy(FRoot.HashValue, 1, 12), '...');
      Exit;
    end;

    // Combinar hasta que quede uno solo
    ...
    begin
      ...
      try
        ...
        // Si queda impar, duplicar el último hash
    ...
    FRoot := TMerkleNode(Leaves[0]);
    Writeln('✅ Árbol de Merkle creado. Hash raíz: ', Copy(FRoot.HashValue, 1, 16), '...');
  finally
    Leaves.Free;
  end;
end;
...
begin
  if FRoot = nil then
  begin
    Writeln('⚠️ Árbol vacío, no se puede graficar.');
    Exit;
  end;
  ...
  Writeln('📄 Gráfico generado en: ', pngFile);
end;
