...
uses
  ...
  ComCtrls, Menus, Unit3, matrix, listsL,bts,block,logclass;
...
type
  ...
  public
    ...
    relations: MatrixL;
    comunities: ListList;
    communitiesTree: BSTree;
    blockChain: TBlockChain;
    logManager: LogListManager;
  end;
...
begin
  ...
  if (userEdit.Text = 'root@edd.com') and (passEdit.Text = 'root123') then
     ...
     ShowMessage('Bienvenido');
     Form3.Show;
     acc.entrada:=FormatDateTime('dd/mm/yyyy  hh:nn:ss',Now);
     Form3.setUser(acc);
     Form1.Hide;
     end
  else
...
begin
  ...
  communitiesTree := BSTree.Create;
  blockChain := TBlockChain.Create;
  logManager := LogListManager.Create;
end;
