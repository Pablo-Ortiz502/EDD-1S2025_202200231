...
type
  ...
  TForm3 = class(TForm)
    ...
    procedure privButtonClick(Sender: TObject);
...
uses Unit1,Unit6,trashUI,addCon,postUI,contactsUI,sendUI,progUI,programedUI,Unit5,prototypeUI,favUI,deleteConUI,sendComUi,privUi,merckeTree;
{$R *.lfm}
...
begin
   Form1.userEdit.Text := '';
   Form1.passEdit.Text := '';
   aLogUser.salida:= FormatDateTime('dd/mm/yyyy  hh:nn:ss',Now);
   Form1.logManager.AddLog(alogUser.Email,aLogUser.entrada,aLogUser.salida);
   Form1.Show;
   Form3.Hide;
end;
...
procedure TForm3.privButtonClick(Sender: TObject);
begin
  Form20.Show;
  Form20.setUser(aLogUser);
  Form3.Hide;
end;
...
procedure TForm3.protoButtonClick(Sender: TObject);
begin
   Form15.Show;
   Form15.setUser(aLogUser);
   Form3.Hide;
end;
...
begin
   ...
   if aLogUser.favTree.aRoot <> nil then
   begin
    list :=  aLogUser.favTree.ToDoubleList;
    aLogUser.favTree.GenerateGraph(aLogUser.name +'__Fasvoritos');
    aLogUser.privTree := TMerkleTree.Create(list);
    aLogUser.privTree.GraphMerkleTree(aLogUser.name +'__Privados');
...
procedure TForm3.comButtonClick(Sender: TObject);
begin
  Form18.Show;
  Form18.setUser(aLogUser);
  Form3.Hide;
end;
